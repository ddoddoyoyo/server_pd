<?php
	...
	//백분율
	//Total
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);

	//1번
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3 AND PD_CHOICE = 1";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
	$row_val1 = $stmt->fetchAll(PDO::FETCH_ASSOC);

	//2번
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3 AND PD_CHOICE = 2";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
	$row_val2 = $stmt->fetchAll(PDO::FETCH_ASSOC);

	//3번
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3 AND PD_CHOICE = 3";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
	$row_val3 = $stmt->fetchAll(PDO::FETCH_ASSOC);

	//4번
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3 AND PD_CHOICE = 4";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
	$row_val4 = $stmt->fetchAll(PDO::FETCH_ASSOC);

	//5번
	$sql = "SELECT COUNT(*) AS CNT FROM HD_PD_CHOICE_INFO WHERE PD_GUBUN = 3 AND PD_CHOICE = 5";
	$stmt = $dbh->prepare($sql);
	$stmt->execute();
